...
public class Main{
    ...
    protected void cmdLine(String[] args) throws Exception {
        ...
        if (args == null || args.length < 1) {
            if (configFiles == null)
                configFiles = Arrays.asList("VFPMF.conf");
            return;
        }

        LineConfiger paramOptions = new LineConfiger(args);

        configFiles = paramOptions.contains("-c") ? paramOptions.getOptions("-c") : Arrays.asList("VFPMF.conf");
    ...
    protected Recommender getRecommender(SparseMatrix[] data, int fold) throws Exception {
        ...
        switch (algorithm.toLowerCase()) {
            case "vfpmf":
                return new VFPMF(trainMatrix, testMatrix, fold);
            default:
